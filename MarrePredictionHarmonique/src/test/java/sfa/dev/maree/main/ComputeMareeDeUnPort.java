...
public class ComputeMareeDeUnPort {
	public static void main(String[] args) throws Exception {
		...
		long epoch = MareeEnv._sdfUI.parse("2021/08/11 09:40:00").getTime();		
		...
		long epochMinuit = MareeEnv._sdfUI.parse("2021/08/11 00:00:01").getTime();	
		...
		for (int i = 0; i < infomaree.size(); i++)
			System.out.println("UTC " + infomaree.get(i).toString());
		...
		{
			System.out.println("UTC " + PM.get(i).toString());
		}
		System.out.println("Les BMs:");
		for (int i = 0; i < BM.size(); i++)
		{
			System.out.println("UTC " + BM.get(i).toString());
		}
